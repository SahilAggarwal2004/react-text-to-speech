...
export default function App() {
  const {
    ...
    stableText: true, // Enable if 'text' is stable across renders for better performance
  });
...
The `highlightText`, `showOnlyHighlightedText`, `highlightMode`, `highlightProps` and `highlightContainerProps` props, available in both the `useSpeech` hook and `<Speech>` component, can be used to highlight text and modify its styling during speech.
...
- **`highlightContainerProps`**: Defines the styling for the container wrapping the highlighted text (useful for adding padding, borders, or background effects).
...
<Tabs>
  <TabItem value="npm" label="npm">
    ```bash npm install react-remarkify rehype-raw rehype-sanitize remark-gfm --save ```
  </TabItem>
  <TabItem value="yarn" label="yarn">
    ```bash yarn add react-remarkify rehype-raw rehype-sanitize remark-gfm ```
  </TabItem>
  <TabItem value="pnpm" label="pnpm">
    ```bash pnpm add react-remarkify rehype-raw rehype-sanitize remark-gfm ```
  </TabItem>
  <TabItem value="bun" label="bun">
    ```bash bun add react-remarkify rehype-raw rehype-sanitize remark-gfm ```
  </TabItem>
</Tabs>
